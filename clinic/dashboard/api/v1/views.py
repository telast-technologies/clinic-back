...
class VisitDashboardView(PandasSimpleView):
    ...
    def get_data(self, request, start_date, end_date, *args, **kwargs):
        ...
        #services
        services_distribution = list(ChargeService.objects.filter(
            invoice__visit__patient__clinic=current_clinic,
            invoice__visit__date__range=(start_date, end_date)
        ).values('service__name').annotate(count=Count('uid')))
        # invoices
        invoices = Invoice.objects.filter(
            visit__patient__clinic=current_clinic,
            ...
            [
                {
                    ...
                    "supplies": {"below_threshold": supplies_below_threshold.count(), "expires_soon": expires_soon.count()},
